...
public class ConfigureABusiness {
  ...
  public static Business createABusinessAndLoadALotOfData(String name, int supplierCount, int productCount,
    ...
    // Add 20 Products to each suppliers(5)
    loadProducts(business, productCount);

    // Add 30 Customers
    loadCustomers(business, customerCount);

    // Add 5 Orders with 1 items (each consumer)
  ...
  static void loadProducts(Business b, int productCount) {
    ...
    // Added to change suppliers to random 5
    for (int index = 1; index <= 5; index ++) {
      int randomNumber = getRandom(0, supplierList.size());
      Supplier supplier = supplierList.get(randomNumber);

      ProductCatalog productCatalog = supplier.getProductCatalog();

      for (int i = 1; i <= productCount; i++) {

        String productName = "Product #" + i + " from " + supplier.getName();
  ...
  static void loadOrders(Business b, int orderCount, int itemCount) {
    ...
    for (int index = 0; index < orderCount; index++) {
      CustomerProfile randomCustomer = cd.customerListGetter().get(index);
      if (randomCustomer == null) {
        System.out.println("Cannot generate orders. No customers in the customer directory.");
        return;
      }
      int randomOrderNumber = getRandom(5, 15);
      for (int n = 0; n < randomOrderNumber; n++) {
        ...
        // System.out.println("randome Item count numner: " + randomItemCount);
